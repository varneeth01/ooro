...
function Checkout({ plan, close }: { plan: Plan; close: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 p-4"><div className="mx-auto mt-8 max-w-lg bg-[#f7f7f4] p-6 text-black sm:p-8"><div className="flex items-start justify-between"><div><p className="eyebrow text-neutral-500">{plan.city} · {plan.durationDays} days</p><h3 className="mt-3 text-2xl font-medium">Your campaign details</h3><p className="mt-2 text-sm text-neutral-500">{plan.autos} autos · {plan.hoursPerDay} hours/day · ₹{plan.amount.toLocaleString("en-IN")}</p></div><button onClick={close} className="text-2xl text-neutral-400" aria-label="Close">×</button></div><form onSubmit={submit} className="mt-7 space-y-4"><input required name="name" placeholder="Contact name" className="w-full border border-neutral-300 bg-white p-3 text-sm"/><input required name="businessName" placeholder="Brand / company name" className="w-full border border-neutral-300 bg-white p-3 text-sm"/><input required type="tel" name="phone" placeholder="+91 98765 43210" autoComplete="tel" className="w-full border border-neutral-300 bg-white p-3 text-sm"/><input required type="email" name="email" placeholder="Email address" className="w-full border border-neutral-300 bg-white p-3 text-sm"/><input required name="city" defaultValue={plan.city} placeholder="City" className="w-full border border-neutral-300 bg-white p-3 text-sm"/><textarea name="campaignNotes" placeholder="Campaign notes (optional)" rows={3} className="w-full border border-neutral-300 bg-white p-3 text-sm"/><p className="text-xs leading-5 text-neutral-500">By continuing, you agree to OORO&apos;s Terms and Privacy Policy and consent to being contacted about this campaign purchase.</p>{error && <p className="text-sm text-red-700">{error}</p>}<button disabled={busy} className="w-full rounded-[7px] bg-black px-4 py-3.5 text-sm font-medium text-white disabled:opacity-50">{busy ? "Preparing secure payment…" : `Continue to pay ₹${plan.amount.toLocaleString("en-IN")}`}</button></form></div></div>;
}
