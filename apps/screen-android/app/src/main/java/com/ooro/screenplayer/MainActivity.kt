...
@Composable private fun Admin(vm:ScreenViewModel){Box(Modifier.fillMaxSize().background(Color(0xFF101010)),contentAlignment=Alignment.Center){Column(horizontalAlignment=Alignment.CenterHorizontally){Text("Technician diagnostics",color=Yellow,fontSize=28.sp);Text("Device: ${vm.deviceId}",color=Color.White);Text("Device Owner: ${OoroDeviceAdminReceiver.isOwner(LocalContext.current)}",color=Color.White);Spacer(Modifier.height(16.dp));Button({vm.sync()}){Text("Refresh manifest")};Button({vm.unpair()}){Text("Unpair screen")};TextButton({vm.closeAdmin()}){Text("Close")}}}}
