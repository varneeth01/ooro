...
class Media3CreativePlayer(context: Context, private val onStarted: () -> Unit = {}, private val onCompleted: (Long) -> Unit = {}, private val onFailed: (Throwable) -> Unit = {}, private val onStall: () -> Unit = {}, private val onPosition: (Long) -> Unit = {}) : AutoCloseable {
    ...
    private val watchdogTick = object : Runnable { override fun run() { if (player.isPlaying) { val position = player.currentPosition; onPosition(position); if (position == lastPosition) { stallCount++; if (stallCount >= 2) { stallCount = 0; recover(); onStall() } } else { lastPosition = position; stallCount = 0 } }; watchdog.postDelayed(this, 10_000) } }
