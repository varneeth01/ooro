...
object ManifestCodec {
    ...
    fun encode(manifest: DeviceManifest): String = JSONObject().put("version", manifest.version).put("screenId", manifest.screenId).put("timezone", manifest.timezone).put("layout", manifest.layout.name).put("validUntil", manifest.validUntil?.toString()).put("items", org.json.JSONArray(manifest.items.map { JSONObject().put("id", it.id).put("campaignId", it.campaignId).put("creativeId", it.creativeId).put("assetId", it.assetId).put("type", it.type.name.lowercase()).put("url", it.url).put("checksum", it.checksum).put("durationSeconds", it.durationSeconds).put("priority", it.priority).put("startAt", it.startAt?.toString()).put("endAt", it.endAt?.toString()).put("daysOfWeek", org.json.JSONArray(it.daysOfWeek.map { day -> day.value })).put("startTime", it.startTime?.toString()).put("endTime", it.endTime?.toString()) })).toString()
}
