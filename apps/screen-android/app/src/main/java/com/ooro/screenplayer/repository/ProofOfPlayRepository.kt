...
class ProofOfPlayRepository(private val db: OoroDatabase) {
    suspend fun record(event: ProofOfPlay) = withContext(Dispatchers.IO) { db.proofDao().add(ProofEntity(event.eventId, JSONObject().put("eventId", event.eventId).put("screenId", event.screenId).put("deviceId", event.deviceId).put("campaignId", event.campaignId).put("creativeId", event.creativeId).put("scheduleItemId", event.scheduleItemId).put("assetId", event.assetId).put("startedAt", event.startedAt.toString()).put("endedAt", event.endedAt.toString()).put("expectedDurationMs", event.expectedDurationMs).put("actualPlayedMs", event.actualPlayedMs).put("success", event.success).put("failureReason", event.failureReason).put("appVersion", event.appVersion).toString())) }
