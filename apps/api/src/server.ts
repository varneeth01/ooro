...
app.get('/api/displays/:displayId/status', async (request, reply) => { const auth = await requireAuth(request); const { displayId } = request.params as { displayId: string }; const display = await prisma.display.findUnique({ where: { id: displayId }, include: { vehicle: true, heartbeats: { orderBy: { occurredAt: 'desc' }, take: 1 } } }); if (!display) throw notFound('Display not found'); if (display.vehicle && auth.driverId !== display.vehicle.driverId && !['ADMIN','SUPER_ADMIN','OPERATIONS'].includes(auth.role)) throw forbidden(); const hb = display.heartbeats[0]; const age = hb ? (Date.now() - hb.occurredAt.getTime()) / 1000 : Infinity; const connectivity = age < config.heartbeatOnlineSeconds ? 'ONLINE' : age < config.heartbeatDegradedSeconds ? 'DEGRADED' : 'OFFLINE'; return ok(request, { ...asJson(display), connectivity, lastHeartbeat: hb ? asJson(hb) : null }, reply) })
...
app.post('/api/device/proof-of-play', async (request, reply) => {
  ...
  for (const e of input.events) {
    ...
    if (e.rideId && e.proofId) {
      ...
      await prisma.playbackEvent.create({ data: { eventId: e.eventId ?? e.proofId ?? randomSecret(), displayId: display.id, campaignId: e.campaignId, creativeId: e.creativeId, eventType: 'CREATIVE_COMPLETED', occurredAt: ended ?? started, duration: actual, manifestVersion: e.manifestVersion, playerVersion: e.playerVersion ?? e.appVersion } }).catch((error: any) => error?.code === 'P2002' ? null : Promise.reject(error))
    }
    accepted += 1
  }
