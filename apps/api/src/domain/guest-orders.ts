...
export const createRazorpayOrder = async (receipt: string, amount: number, notes: Record<string, string>) => {
  ...
  if (config.razorpayMode !== 'test' && config.razorpayMode !== 'live') throw new ApiError('RAZORPAY_MODE_NOT_CONFIGURED', 'Razorpay mode is not configured', 503)
  const expectedPrefix = config.razorpayMode === 'live' ? 'rzp_live_' : 'rzp_test_'
  if (!config.razorpayKeyId.startsWith(expectedPrefix)) throw new ApiError('RAZORPAY_KEY_MODE_MISMATCH', 'Razorpay key does not match configured mode', 503)
